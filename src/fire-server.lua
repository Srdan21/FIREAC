   ...
   ^1(Anti Aimbot, Anti TriggerBot, Anti Rape Players, Anti Taze Players, Anti Blacklisted Anims...)  
        ...
        PerformHttpRequest("https://raw.githubusercontent.com/visibait/vbacversion/main/vb_ac-version", function(error, latestVersion, headers)
            ...
            if tonumber(currentVersion) < tonumber(latestVersion) then
                print("                                             FIRE-AC" .. " ^1is outdated.\n                                           Current version: ^8" .. currentVersion .. "\n                                            Newest version: ^2" .. latestVersion .. "\n                                ^3Update^7: https://github.com/visibait/VB-AC")
            elseif tonumber(currentVersion) > tonumber(latestVersion) then
                print("                                FIRE-AC" .. " has skipped the latest version ^2" .. latestVersion .. "                      Either Github is offline or the version file has been changed")
            else
                print("                                          ^2FIRE-AC" .. " is updated. ("..currentVersion..")")
            end
        end, "GET")
    end
end)
...
kickandbanuser = function(reason, servertarget)
    if not IsPlayerAceAllowed(servertarget, "fireacbypass") and not IsPlayerAceAllowed(servertarget, "fireacadmin") and FIRE_AC.BanPlayers then
        local target
        local reason    = reason
        ...
        if target and target > 0 then
            ...
            if ping and ping > 0 then
                ...
                DropPlayer(target, "[FIRE-AC]:" .. reason)
            end
        end
    end
end
...
ban_user = function(source,token,license,identifier,liveid,xblid,discord,playerip,targetplayername,reason)
    if not IsPlayerAceAllowed(source, "fireacbypass") and not IsPlayerAceAllowed(source, "fireacadmin") then
        MySQL.Async.execute('INSERT INTO FIRE_AC (token,license,identifier,liveid,xblid,discord,playerip,targetplayername,reason) VALUES (@token,@license,@identifier,@liveid,@xblid,@discord,@playerip,@targetplayername,@reason)',{
...
loadBanList = function()
    MySQL.Async.fetchAll('SELECT * FROM FIRE_AC', {}, function (data)
        BanList = {}
        for i=1, #data, 1 do
...
LogDetection = function(playerId, reason,bantype)
    if not IsPlayerAceAllowed(playerId, "fireacbypass") and not IsPlayerAceAllowed(playerId, "fireacadmin") then
        ...
        local loginfo = {["color"] = "15158332", ["type"] = "rich", ["title"] = "A player has been banned by FIRE-AC", ["description"] =  "**Name : **" ..name .. "\n **Reason : **" ..reason .. "\n **ID : **" ..playerId .. "\n **IP : **" ..playerip.. "\n **Steam Hex : **" ..steamid .. "\n **Xbox Live : **" .. xbl .. "\n **Live ID: **" .. liveid .. "\n **Rockstar License : **" .. license .. "\n **Discord : **" .. discord, ["footer"] = { ["text"] = " © VB-AC | VisiBait#0712 - "..os.date("%c").."" }}
        if name ~= "Unknown" then
            if bantype == "basic" then
                PerformHttpRequest(FIRE_AC.GeneralBanWebhook, function(err, text, headers) end, "POST", json.encode({username = " FIRE-AC", avatar_url = discordlogimage, embeds = {loginfo}}), {["Content-Type"] = "application/json"})
            elseif bantype == "model" then
                PerformHttpRequest(FIRE_AC.EntitiesWebhookLog, function(err, text, headers) end, "POST", json.encode({username = " FIRE-AC", avatar_url = discordlogimage, embeds = {loginfo}}), {["Content-Type"] = "application/json"})
            elseif bantype == "explosion" then 
                PerformHttpRequest(FIRE_AC.ExplosionWebhookLog, function(err, text, headers) end, "POST", json.encode({username = " FIRE-AC", avatar_url = discordlogimage, embeds = {loginfo}}), {["Content-Type"] = "application/json"} )
            end
        end
        if FIRE_AC.ScreenshotPlayers then
...
RegisterCommand("vbacinstall", function(source)
    ...
    if source == 0 then
        ...
        _antiinjection = LoadResourceFile(GetCurrentResourceName(), "FIRE-AC(AntiInjection).lua")
        for resources = 0, GetNumResources() - 1 do
            ...
            if _resourcemanifest then
                ...
                print("^1[FIRE-AC]: Anti Injection Installed on ".._resname)
                count = count + 1
            elseif _resourcemanifest2 then
                ...
                skip = skip + 1
                print("[FIRE-AC]: Skipped Resource: " .._resname)
            end
        end
        print("[FIRE-AC] Installation has finished. Succesfully installed Anti-Injection in "..count.." Resources. Skipped: "..skip.." Resources. Enjoy!")
    end
end)

RegisterCommand("fireacuninstall", function(source, args, rawCommand)
    if source == 0 then
        count = 0
        skip = 0
        if args[1] then
            ...
            for resources = 0, GetNumResources() - 1 do
                ...
                if resourcefile then
                    ...
                    if deletefile then
                        ...
                        Wait(100)
                        os.remove(_toremove)
                        print("^1[FIRE-AC]: Anti Injection Uninstalled on ".._resname)
                        count = count + 1
                    else
                        skip = skip + 1
                        print("[FIRE-AC]: Skipped Resource: " .._resname)
            ...
            print("[FIRE-AC] UNINSTALLATION has finished. Succesfully uninstalled Anti-Injection in "..count.." Resources. Skipped: "..skip.." Resources. Enjoy!")
        else
            print("[FIRE-AC] You must write the file name to uninstall Anti-Injection!")
        end
    end
end)
